    ...
    <PageLayout showFooter={false} className="px-8 md:px-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10 animate-fadeInUp">
        ...
        <Card padding="normal" className="bg-gradient-to-br from-[#1a1d26] to-[#1a1d26] border-[#ffb4ab]/40 shadow-[0_0_30px_rgba(255,180,171,0.15)] glow-error">
          ...
          {data?.conjunctions?.conjunctions?.[0] ? (
        ...
        <Card padding="normal" className="shadow-[0_0_20px_rgba(79,219,200,0.1)] border-t-[#4fdbc8]/30">
          ...
          {decisions[0] ? (
            <div className="space-y-2">
              ...
              <div className="text-[0.7rem] text-[#bbcac6] leading-relaxed line-clamp-3">
                {decisions[0].explanation || decisions[0].deferred_reason || 'Decision made'}
              </div>
            </div>
          ) : (
            <div className="text-center py-4">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-bold text-[#e1e2eb]">
                  {computedData.maneuvers[0].satellite_id}
                </span>
                <div 
                  className="px-2 py-1 rounded text-[0.65rem] font-bold uppercase"
                  style={{
                    backgroundColor: getManeuverStatusColor(computedData.maneuvers[0].status).bg,
                    color: getManeuverStatusColor(computedData.maneuvers[0].status).text
                  }}
                >
                  {computedData.maneuvers[0].status}
                </div>
              </div>
              <div className="space-y-1">
                <div className="flex justify-between text-[0.7rem]">
                  <span className="text-[#bbcac6]">Delta-V:</span>
                  <span className="font-mono text-[#4fdbc8] font-semibold">
                    {formatDeltaV(computedData.maneuvers[0].delta_v)}
                  </span>
                </div>
                <div className="flex justify-between text-[0.7rem]">
                  <span className="text-[#bbcac6]">Fuel Cost:</span>
                  <span className="font-mono text-[#e1e2eb] font-semibold">
                    {formatFuel(computedData.maneuvers[0].estimated_fuel_kg)}
                  </span>
                </div>
              </div>
            </div>
...
function formatMissDistance(meters) {
...
function formatDeltaV(deltaV) {
  if (deltaV == null) return '—';
...
function getDecisionColor(decision) {
  if (decision?.maneuver_planned) {
    return { bg: '#4fdbc8', text: '#003731' };
  }
  if (decision?.deferred_reason) {
    return { bg: '#ffb59e', text: '#1a1d26' };
  }
  return { bg: '#bbcac6', text: '#1a1d26' };
}

function getManeuverStatusColor(status) {
  const statusUpper = status?.toUpperCase();
  if (statusUpper === 'SCHEDULED') {
    return { bg: '#5b9ff9', text: '#ffffff' };
  }
  if (statusUpper === 'EXECUTING') {
    return { bg: '#ffb59e', text: '#1a1d26' };
  }
